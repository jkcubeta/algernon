...
from overwatch.alg_tasks import mongo_tasks
...
def batch_adjust_notes(results, approve=False):
    cookie_jar = get_cbh_cookie()
    ...
    for result_dict in results:
        if result_dict != 0:
            ...
            if arbitration['approve'] and approve:
...
def set_single_red_x(service_package, red_x_package, session, cookie_jar):
    ...
    mongo_tasks.log_overwatch_action(
        'service',
        service_package['clientvisit_id'],
        'set red x',
        str(red_x_package)
    )
    return data
...
def update_single_client_batch(client_id, updates, cookie_jar):
    ...
    mongo_tasks.log_overwatch_action('client', client_id, 'update profile', updates)
    return 1
...
def update_client(client_id, field, value):
    ...
    requests.post(url, data=urlencode(data), cookies=cookie_jar, params=params, headers=header)
    mongo_tasks.log_overwatch_action('client', client_id, 'update profile', {field: value})
    logout(cookie_jar)
    return 1
